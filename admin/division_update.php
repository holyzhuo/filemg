...
<?php require_once 'session_check.php';
      ...
      $sql = "select * from division where division_id='".$result->escapeString($_GET['id'])."'";
      $divisionrow = $result->getRow($sql);
?>
<!DOCTYPE html>
<html lang="zh-CN">
  <head>
  ...
  <!--头部导航条-->
    ...
    <div class="left-body">
      ...
      <div class="rigth-body">
          <form name="form1" onsubmit="return myCheck()" class="form-horizontal text-center" method="post" action="division_update_check.php">
          <input type="hidden" name="division_id" value="<?php echo $divisionrow['division_id']; ?>">
                <span style="margin:200px 0 200px 200px;padding-left:170px;float: left;">部门名称</span>
                <div class="col-xs-2" style="text-align: center;margin-top: 195px;">
              <input type="text" class="form-control" name='divisionname' value="<?php echo $divisionrow['division_name'] ;?>" />
                </div>
                <button type="submit" class="btn btn-primary" style="margin-top: 195px;">修改</button>
          </form>
      </div>
    </div>
    <div>
